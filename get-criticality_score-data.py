...
def update_criticality(id, name, project_version):
    ...
    criticality_file = open("/home/arun/besecure-assessment-datastore/"+name+"/"+project_version+"/criticality_score/"+name+"-"+project_version+"-criticality_score-report.json","r")
    criticality_data = json.load(criticality_file)
    
    criticality_score = criticality_data["criticality_score"]
    
    version_file = open("/home/arun/BeSLighthouse/bes_theme/assets/data/version_details/"+str(id)+"-"+name+"-Versiondetails.json", "r+")
...
def write_criticality_score_data(id, name):
    ...
    criticality_file = open("criticality_score.json", "r")    
    criticality_data = json.load(criticality_file)
